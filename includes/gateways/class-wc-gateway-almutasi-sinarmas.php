...
class WC_Gateway_Almutasi_SINARMAS extends Almutasi_Gateway
{
    public $sub_id = 'almutasi_woocommerce_sinarmas';
    
    public function __construct()
    {
        parent::__construct();

        $this->method_title = "alMutasi - Bank Sinarmas";
        $this->method_description = "Pembayaran divalidasi otomatis berdasarkan nominal unik";
        $this->payment_method = "sinarmas";
        ...
        if ($this->settings['enable_icon'] == 'yes') {
            $this->icon = !empty($this->settings['custom_icon'])
                ...
                : plugins_url('/assets/sinarmas.png', dirname(__FILE__));
        }
    }
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            ...
            'enable_icon' => array(
                ...
                'label' => '',
                'type' => 'checkbox',
                'description' => '<img src="'.plugins_url('/assets/sinarmas.png', dirname(__FILE__)).'" style="height:100%;max-height:40px !important" />',
                'default' => 'no',
            ),
            'custom_icon' => array(
