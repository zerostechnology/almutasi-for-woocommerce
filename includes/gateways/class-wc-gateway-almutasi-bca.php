...
class WC_Gateway_Almutasi_BCA extends Almutasi_Gateway
{
    public $sub_id = 'almutasi_woocommerce_bca';
    
    public function __construct()
    {
        parent::__construct();

        $this->method_title = "alMutasi - Bank BCA";
        $this->method_description = "Pembayaran divalidasi otomatis berdasarkan nominal unik";
        $this->payment_method = "bca";
        ...
        if ($this->settings['enable_icon'] == 'yes') {
            $this->icon = !empty($this->settings['custom_icon'])
                ...
                : plugins_url('/assets/bca.png', dirname(__FILE__));
        }
    }
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            ...
            'enable_icon' => array(
                ...
                'label' => '',
                'type' => 'checkbox',
                'description' => '<img src="'.plugins_url('/assets/bca.png', dirname(__FILE__)).'" style="height:100%;max-height:40px !important" />',
                'default' => 'no',
            ),
            'custom_icon' => array(
